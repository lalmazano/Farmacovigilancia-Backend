...
builder.Services.AddScoped<IEfectoAdversoService, EfectoAdversoService>();
...
builder.Services.AddScoped<IEfectoAdversoRepository, EfectoAdversoRepository>();
builder.Services.AddScoped<IDetalleRecetasRepository, DetalleRecetasRepository>();
...
{
    options.UseOracle(encriptador.DesencriptarData(configuration.GetConnectionString("OracleQueryUser")), x => x.UseOracleSQLCompatibility(OracleSQLCompatibility.DatabaseVersion21));
});
...
builder.Services.AddDbContext<OperationContext>((serviceProvider, options) =>
{
    ...
    options.UseOracle(encriptador.DesencriptarData(configuration.GetConnectionString("OracleOperationUser")), x => x.UseOracleSQLCompatibility(OracleSQLCompatibility.DatabaseVersion21));
